...
export function TailwindFormClone() {
  ...
  useEffect(() => {
    performance.mark('tailwindClone-mount-start');
    const timer = setTimeout(() => {
      performance.mark('tailwindClone-mount-end');
      try {
        performance.measure('tailwindClone-mount', 'tailwindClone-mount-start', 'tailwindClone-mount-end');
      } catch (e) {
  ...
  useEffect(() => {
    performance.mark('tailwindClone-update-start');
    setTimeout(() => {
      performance.mark('tailwindClone-update-end');
      try {
        performance.measure('tailwindClone-update', 'tailwindClone-update-start', 'tailwindClone-update-end');
      } catch (e) {
        // May fail if marks are cleared or not set, safe to ignore
      }
    }, 0);
  ...
  function onSubmit(data: FormValues) {
    toast({
      title: 'Tailwind Form (Clone) Submitted',
      description: (
  ...
  const onFinalSubmit = form.handleSubmit(
    (data) => {
      onSubmit(data);
      performance.mark('tailwindClone-submit-end');
      performance.measure(
        'tailwindClone-submit',
        'tailwindClone-submit-start',
        'tailwindClone-submit-end'
      );
    },
    () => {
      performance.mark('tailwindClone-submit-end');
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    performance.mark('tailwindClone-submit-start');
    onFinalSubmit(e);
  };
